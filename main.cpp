...
float MS_PER_INCH = 95.0;          //time to drive one inch
unsigned long MS_PER_90_TURN = 500; // time to pivot-turn 90 degrees
...
void setup() {
  ...
  Serial.println("Starting navigation (no gyro, timed turns/drives).");
  delay(1000);
}
...
void turnToHeading(Direction target) {
  int diff = ((int)target - (int)currentHeading + 4) % 4; 
